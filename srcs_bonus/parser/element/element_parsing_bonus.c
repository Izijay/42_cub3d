...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   element_parsing_bonus.c                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: mdupuis <mdupuis@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/06/06 13:30:18 by ranuytte          #+#    #+#             */
/*   Updated: 2022/06/06 14:30:26 by mdupuis          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d_bonus.h"

static int	check_nb_element_texture(char **info)
{
	if (!info[1])
		return (1);
	if (info[2] && *info[2] != '\n')
...
static int	check_duplicate(t_data *data, char **sp)
{
	int	i;
	...
	while (++i < 6)
	{
		if (!ft_strstr(data->elem[i].id, sp[0]))
		{
			if (data->elem[i].path)
				return (1);
...
static int	identify_identifier(char *line)
{
	int	i;
...
static int	check_identifier(t_data *data, char *line, char ***split)
{
	if (identify_identifier(line))
		return (1);
	*split = ft_split(line, ' ');
...
int	element_parsing(t_data *data, int fd)
{
	...
	tmp = line;
	spaces_do_not_matter(&line);
	while (!spaces_do_not_matter(&line) && *line == '\n')
	{
		free(tmp);
		tmp = NULL;
		line = get_next_line(fd);
